                       ...
                       hw_device_t      **device)
{
    ...
    ALOGE("camera_device open: %s", name);
               ...
               unsigned int pathLen)
{
    ...
    ALOGE("GetDevPath pCameraName:%s pCameraDevPath:%s",pCameraName,pCameraDevPath);
    ...
    if (v4l_dir) {
        while ((dir_entry = readdir(v4l_dir))) {
            ...
            if (ioctl(fd, VIDIOC_QUERYCAP, &v4l2_cap) < 0) {
                close(fd);
                fd = 0;
                ALOGE("GetDevPath fail dev_node:%s",dev_node);
                continue;
            } else if (v4l2_cap.capabilities & V4L2_CAP_VIDEO_CAPTURE) {
                ALOGE("GetDevPath v4l2_cap.driver,:%s",(const char*)v4l2_cap.driver);
                if (ioctl(fd, VIDIOC_DBG_G_CHIP_IDENT, &vid_chip) < 0) {
                    if(strstr((const char*)v4l2_cap.driver, pCameraName)) {
                       if (pathLen > strlen(dev_node)) {

                            if(gCameraNum == 1){
                                ALOGE("GetDevPath sCameraInfo[%d].devPath:%s",gCameraNum-1,
                                        sCameraInfo[gCameraNum-1].devPath);
                                if(strstr(sCameraInfo[gCameraNum-1].devPath,dev_node)){
                                    close(fd);
                                    fd = 0;
                                    continue;
                                }
                            ...
                            ALOGE("Get sensor %s's dev path %s",
                                  pCameraName,
                                  pCameraDevPath);
                            retCode = 0;
                        }
                ...
                ALOGE("GetDevPath vid_chip.match.name:%s",vid_chip.match.name);
    ...
    if (gCameraNum == 0) {
        ...
        if (name_back[0] != DEFAULT_ERROR_NAME) {
            ...
            while (pCameraName != NULL) {
                ...
                ALOGE("GetDevPath Camera ID %d: name %s, Facing %d, orientation %d, dev path %s",
                        gCameraNum,
